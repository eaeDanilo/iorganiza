...
export default async function DashboardHomePage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-4xl font-bold tracking-tight">Olá, {user.full_name || 'usuário'}</h1>
        <p className="mt-2 text-muted-foreground">Resumo das suas assinaturas.</p>
      </div>

      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <StatCard label="SaaS ativos" value={active.length} icon={<Package className="h-5 w-5" />} accent="primary" />
        <StatCard
          label="Total mensal"
      ...
      <div className="mt-10">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-2xl font-semibold">Seus SaaS</h2>
        ...
        {list.length === 0 ? (
          <Card>
            <CardContent className="p-12 text-center">
              <p className="text-muted-foreground">Você ainda não assinou nenhum SaaS.</p>
              ...
              <Card key={s.id} className="group transition hover:scale-[1.02] hover:shadow-glow">
                ...
                <CardContent>
                  ...
                  <Button asChild className="mt-4 w-full">
                    <Link href={s.saas.external_url || `/dashboard/meus-saas`}>Acessar</Link>
                  </Button>
                </CardContent>
              </Card>
            ))}
